...
app.directive('cwSelectstr2t', ["$timeout", function ($timeout) {
    return {
        require: '?ngModel',
        restrict: 'EA',
        transclude: true,
        scope: {
            cwtext: '=', cwmultiple: '=', selectdata: '=', selectid: '='
        },
        link: function ($scope, elem, attr, ngModel) {
            $scope.newData = [];
            var is_mult = $scope.cwmultiple;
            $scope.$watch('selectdata', function (a) {
                if (a != undefined) {
                    $scope.newData = a;
                    $(elem).select2({data: a, multiple: is_mult});
                    $(elem).select2('val', is_mult ? $scope.selectid.split(",") : $scope.selectid)
                    $scope.$watch('selectid', function (a) {
                        if (a != undefined && a != '') {
                            if ($scope.cwtext !== undefined) {
                                if (is_mult) {
                                    $scope.cwtext = $(elem).select2('data')
                                }
                                else {
                                    $scope.cwtext = $(elem).select2('data').text;
                                }
                            }
                        }
                    })
...
app.directive('cwTopMenu', function ($compile) {
    return {
        ...
        link: function ($scope, element, attrs, ngModel) {
            ...
            $scope.menuList = $scope.$eval($scope.topMenuOptions.data);
            $scope.menuIndex = 0;
            $scope.target = '';
